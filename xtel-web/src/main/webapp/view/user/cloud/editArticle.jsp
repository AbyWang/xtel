...
<form class="form-horizontal" id="article">
        <div class="form-group">
         <label class="col-sm-1 control-label">修改文库</label></br>
 			<hr style="color:red;">
         	  <input type="hidden"  id="id" value="${article.id}"/>
              <label class="col-sm-1 control-label">文章名称:</label>
              <div class="col-sm-4">
               <input type='text' name="name" id="name"  class="form-control" value="${article.name}" datatype="*" nullmsg="请输入文章名称!"   placeholder="请输入文章名称" />
               </div>
        </div>
        ...
        <div class="form-group"> 
           ...
           <div id="contentValue" style="display:none;">${article.content}</div>
...
<script type="text/javascript">
  ...
  $(function(){
      ...
      //假定你的信息提示方法为showmsg， 在方法里可以接收参数msg，当然也可以接收到o及cssctl;
       ...
       editor.txt.html($("#contentValue").html());//向文本域设值
  });
    
	function doAdd(){
		 $.ajax({
			    type:"post",
			 	url: path+'/library/updateArticle',
			    data: {id:$("#id").val(),name:$("#name").val().trim(),content:editor.txt.html()},
