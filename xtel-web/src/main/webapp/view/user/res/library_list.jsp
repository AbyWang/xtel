...
function loadTable(flag){
    ...
    var table = new BSTable("listMyLibrary",path+ "/library/listAllLibraryPage", defaultColunms);
    table.init();
    if(flag==1){
        table.refresh();
    }	
    ...
    return [
        ...
        {title: '操作', align: 'center', valign: 'middle',width:'50px', formatter: 
  		     function (value, row, index) {
        		return '<a id="btn_add" style="padding: 1px 5px;font-size:12px;line-height:1.5;border-radius:3px;" type="button" class="btn btn-info btn-sm" href="<%=path%>/library/toArtileDetailPage/'+row.id+'">查看</a>&nbsp;';
        }
        }]
    };
